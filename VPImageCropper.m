...
- (void)showAllPic{
    ...
    if (roti>1) {
        if (frame.size.height > Screen_width*1) {
            frame.size.height = Screen_width*1;
            frame.size.width = (Screen_width*1)/roti;
        }
    }else{
...
//    self.showImgView.size = CGSizeMake(320, 480);
//    self.showImgView.frame = [self.showImgView convertRect:self.showImgView.frame toView:self.finalView];
...
- (void)initControlBtn {
    ...
    [self.rotateButton setTitle:@"取消" forState:UIControlStateNormal];
    [self.rotateButton setTitleColor:COLOR_MAIN_GREEN forState:UIControlStateNormal];
//    [self.rotateButton setImage:[UIImage imageNamed:@"rotate"] forState:UIControlStateNormal];
    ...
    [self.clipButton setTitle:@"确定" forState:UIControlStateNormal];
    [self.clipButton setTitleColor:COLOR_MAIN_GREEN forState:UIControlStateNormal];
//    [self.clipButton setImage:[UIImage imageNamed:@"clip_selected"] forState:UIControlStateNormal];
...
//    return image;
    ...
    //-------------> myInmageRect想要截取的区域
    CGRect myImageRect=self.cropFrame;
    CGImageRef subImageRef = CGImageCreateWithImageInRect(imageRef, myImageRect);
